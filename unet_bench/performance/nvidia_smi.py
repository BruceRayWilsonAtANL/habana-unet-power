...
def clean_file(txt_filename, root_dir):
    ...
    header = lines[0]
    #TODO change to time differential once the next run of ThetaGPU generates time data
    idx = 1
    
    # Open file for writing, then write the csv header
    with open(f"{root_dir}/theta-csvs/post-procure/{txt_filename}.txt", 'w') as outfile:
